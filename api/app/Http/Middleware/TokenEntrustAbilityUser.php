...
use DB;
class TokenEntrustAbilityUser extends BaseMiddleware
{
    public function handle($request, Closure $next, $roles, $permissions, $validateAll = false)
    {

        //$token = $token = $this->auth->setRequest($request)->getToken();// isset(json_decode($request->getContent())->token)?json_decode($request->getContent())->token : '';
        $token = isset(json_decode($request->getContent())->token)?json_decode($request->getContent())->token : '';
        ...
        try {
            ...
            Log::info("Token", ["token :" => $user, "time" => date('H:m:s')]);
